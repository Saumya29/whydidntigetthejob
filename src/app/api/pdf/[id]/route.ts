...
export async function GET(
	...
	// ATS Score
	if (result.atsScore) {
		addSectionTitle(`🤖 ATS Score: ${result.atsScore.score}/100`);
	...
	if (result.competition) {
		addSectionTitle("📊 Competition Analysis");
	...
	if (result.skillGapHeatmap && result.skillGapHeatmap.length > 0) {
		...
		for (const skill of result.skillGapHeatmap) {
			const status = skill.status === "missing" ? "❌" : skill.status === "weak" ? "⚠️" : "✅";
	...
	if (result.priorities && result.priorities.length > 0) {
		addSectionTitle("🚨 Fix This First");
	...
	// The Brutal Truth
	addSectionTitle("🔥 The Brutal Truth");
	addText(result.rejection);
	y += 5;

	// Hiring Manager Quote
	addSectionTitle("💬 What the Hiring Manager Said");
	addText(`"${result.hiringManagerQuote}"`);
	y += 5;

	// Improvements
	if (result.improvements && result.improvements.length > 0) {
		addSectionTitle("💡 How to Actually Get Hired");
	...
	y += 8;
	doc.setFontSize(10);
	doc.text(`Report ID: ${result.resultId}`, pageWidth / 2, y, { align: "center" });
	y += 6;
